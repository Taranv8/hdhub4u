// Add this function to your lib/controllers/movietopController.ts
...
export interface MonthlyMovie {
  ...
  export async function getTopMonthlyMovies(
    ...
    try {
      ...
      // Fetch movies sorted by monthly downloads (highest first)
      // Filter for movies that have been reset this month (or have downloads)
      const moviesFromDb = await db
        .collection('movies')
        .find({
          monthlydownload: { $exists: true, $gt: 0 }
        })
        .sort({ monthlydownload: -1 })
        .limit(limit)
        .toArray();
